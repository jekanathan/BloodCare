...
export default function HospitalActivityLogsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get('/hospital-assets/activity-logs?limit=100')
    ...
    <div className="animate-fade">
      <div className="page-header">
        <div className="page-header-left">
          <h1>Hospital Activity Logs</h1>
          <p>Real registration events and blood request updates across all hospitals</p>
        </div>
      </div>
        ...
        <div className="card-body" style={{ padding: '10px 0' }}>
          ...
          {!loading && logs.map(log => (
            <div key={log.id} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '12px 20px', borderBottom: '1px solid var(--slate-100)' }}>
              ...
              <div style={{ flex: 1, fontSize: 13, color: 'var(--slate-800)' }}>{log.text}</div>
