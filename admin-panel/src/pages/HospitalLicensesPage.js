...
export default function HospitalLicensesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get('/hospital-assets/licenses')
    ...
    <div className="animate-fade">
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p>License numbers, expiry tracking and documents for approved hospitals</p>
        </div>
      </div>
      ...
      <div className="card">
        <div className="table-container">
          <table>
            <thead><tr><th>Hospital</th><th>District</th><th>License No.</th><th>Registration No.</th><th>Expiry Date</th><th>Status</th><th>Document</th></tr></thead>
            <tbody>
              {loading && (<tr><td colSpan={7}><div className="empty-state"><p>Loading...</p></div></td></tr>)}
              {!loading && licenses.map(l => (
                <tr key={l._id}>
                  <td className="td-name">{l.hospitalName}</td>
              ...
              {!loading && licenses.length === 0 && (
                <tr><td colSpan={7}><div className="empty-state"><FileBadge size={36} style={{ margin: '0 auto 12px', opacity: .3 }} /><h3>No approved hospitals yet</h3></div></td></tr>
              )}
            </tbody>
          </table>
        </div>
