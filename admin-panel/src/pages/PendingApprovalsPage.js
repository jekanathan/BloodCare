...
export default function PendingApprovalsPage() {
  ...
  const typeSlug = (type) => type === 'Hospital' ? 'hospital' : type === 'Donor' ? 'donor' : 'bloodbank';
  ...
  const getVerificationItems = (app) => app.type === 'Donor' ? [
    {label:'Required Fields',            status:'Completed',                              done:true},
    {label:'NIC Number',                  status: app.regNumber ? 'Valid' : 'Missing',     done:!!app.regNumber},
    {label:'Blood Group',                 status: app.bloodGroup ? app.bloodGroup : 'Missing', done:!!app.bloodGroup},
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                  <div style={{display:'flex',flexDirection:'column',gap:12}}>
                    {(selected.type==='Donor' ? [
                      ...
                      {label:'NIC Number',        value:selected.regNumber || '—'},
              ...
              <div className="card-body" style={{padding:'18px 22px'}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,flexWrap:'wrap'}}>
                  <div style={{display:'flex',gap:14,flex:1,minWidth:280}}>
                    <div style={{width:48,height:48,borderRadius:'var(--r-md)',background:app.type==='Hospital'?'var(--blue-100)':app.type==='Donor'?'#FEE2E2':'var(--purple-100)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,flexShrink:0}}>
                      {app.type==='Hospital'?'🏥':app.type==='Donor'?'🩸':'🏦'}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4,flexWrap:'wrap'}}>
                        <div style={{fontSize:16,fontWeight:800,color:'var(--slate-900)'}}>{app.name}</div>
                        <span style={{fontSize:11,fontWeight:700,padding:'3px 8px',borderRadius:100,background:app.type==='Hospital'?'var(--blue-100)':app.type==='Donor'?'#FEE2E2':'var(--purple-100)',color:app.type==='Hospital'?'var(--blue-600)':app.type==='Donor'?'var(--red-600)':'#7C3AED'}}>{app.type}</span>
                        {app.type==='Donor' && app.bloodGroup && (
                          <span style={{fontSize:11,fontWeight:700,padding:'3px 8px',borderRadius:100,background:'var(--slate-100)',color:'var(--slate-700)'}}>{app.bloodGroup}</span>
                        )}
        ...
        {!loading && filtered.length===0 && (
          <div className="card">
            <div className="empty-state">
              ...
              <p>{apps.length === 0 ? 'No hospital, blood bank, or donor registrations yet.' : 'Try adjusting your filters'}</p>
            </div>
          </div>
        )}
      </div>
