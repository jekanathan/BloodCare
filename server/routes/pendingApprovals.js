...
function toApplication(record, type) {
  ...
  return {
    ...
    type,
    name: isHospital ? record.hospitalName : record.bankName,
...
router.patch('/:type/:id/approve', auth, async (req, res) => {
  try {
    ...
    const Model = type === 'hospital' ? Hospital : type === 'bloodbank' ? BloodBank : null;
    ...
    if (io) {
      io.to('admin_room').emit('application_approved', { type, id, name: record.hospitalName || record.bankName });
    }
