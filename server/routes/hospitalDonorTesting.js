...
router.get('/appointments', auth, async (req, res) => {
  try {
    // testingBooking.facilityId stores the Hospital document's _id, not the
    // logged-in User's _id — resolve it first or bookings never match.
    ...
    const query = {
      ...
      'testingBooking.facilityId': hospital._id,
    };
...
router.patch('/:donorId/complete-test', auth, async (req, res) => {
  try {
    ...
    if (donor.testingBooking?.facilityType !== 'hospital' || String(donor.testingBooking?.facilityId) !== String(hospital._id)) {
